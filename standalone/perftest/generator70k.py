...
f = open("s1_" + str(now).replace(' ', '_') + "_(" + note + ").csv", "w+")
...
MAX_ANCESTORS_COUNT = 11
MIN_ANCESTORS_COUNT = 9
...
max_job_group_count = int(job_count * 2.0 / 3.0)
wdist1 = "uniform" #get_random_whatever_dist() # We need to preserve the same "whatever" dists for all jobs
wdist2 = "uniform" #get_random_whatever_dist()
wdist3 = "uniform" #get_random_whatever_dist()
wdist4 = "uniform" #get_random_whatever_dist()
...
for job in range(job_count):
    ...
    current_job_busyness_section_count = 4
...
max_worker_group_count = int(worker_count / 2.5)
...
for job_group in job_groups_dict:
    ...
    WORKER_COEFFICIENT = 8500 / worker_count
